...
class UpdateHomePageContentRequest extends FormRequest
{
    ...
    {
        $contentId = $this->route('homepage-content')->id ?? $this->route('homepage-content');

        return [
            'section_key' => ['required', 'string', 'max:255', Rule::unique('home_page_contents', 'section_key')->ignore($contentId)],
