...
class PageController extends Controller
{
    ...
    public function show($slug = null)
    {
        ...
        // Special handling for directors-message page
        if (in_array($slug, ['directors-message', 'governing-body'])) {
            $director = \App\Models\Staff::where('is_active', true)
                ->where(function($query) {
                    $query->where('position', 'like', '%Director%')
                          ->orWhere('position', 'like', '%Chairman%')
                          ->orWhere('position', 'like', '%Founder%');
                })
                ->with('media')
                ->first();
            
            return view('pages.leadership', compact('page', 'director'));
        }
    ...
    public function category(string $pageSlug = null)
    {
        ...
        if ($pageSlug) {
            ...
            // Special handling for directors-message page
            if (in_array($pageSlug, ['directors-message', 'governing-body'])) {
                $director = \App\Models\Staff::where('is_active', true)
                    ->where(function($query) {
                        $query->where('position', 'like', '%Director%')
                              ->orWhere('position', 'like', '%Chairman%')
