...
@php
    ...
    // Pull name/title from Staff model if available, fallback to metadata
    $leaderName = (isset($director) && $director) ? $director->name : ($profileData['leader_name'] ?? 'Hasan Mahmud');
    $leaderTitle = (isset($director) && $director) ? $director->position : ($profileData['leader_title'] ?? 'Founder & Director');
    
    // Try: 1) Staff photo  2) Page profile_image media  3) metadata  4) placeholder
    if (isset($director) && $director && $director->hasMedia('photo')) {
        $leaderImage = $director->getMediaUrl('photo', 'webp') 
                    ?: $director->getMediaUrl('photo', 'medium') 
                    ?: $director->getMediaUrl('photo');
    } elseif ($page->hasMedia('profile_image')) {
        $leaderImage = $page->getFirstMediaUrl('profile_image', 'medium');
    } else {
        $leaderImage = $profileData['leader_image'] ?? asset('images/placeholder.svg');
    }
        ...
        <div class="flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-16">
            ...
            <div class="flex-1 text-center lg:text-left fade-in">
                ...
                <p class="text-base text-gray-600 max-w-xl leading-relaxed">
                    A few words from our {{ $leaderTitle }} reflecting the vision, mission, and values of Duha International School.
                </p>
            </div>
