...
@section('content')
    ...
    @php
        ...
        // Get principal image if available
        $principalImage = null;
        $principalImageWebP = null;
        if ($principal && $principal->hasMedia('photo')) {
            $principalMedia = $principal->getFirstMedia('photo');
            if ($principalMedia) {
                // Try WebP conversion first
                if ($principalMedia->hasGeneratedConversion('webp')) {
                    $principalImageWebP = $principal->getMediaUrl('photo', 'webp');
                }
                // Get large or original image
                $principalImage = $principal->getMediaUrl('photo', 'large') ?: $principal->getMediaUrl('photo');
            }
        }
    @endphp
    ...
    <section class="py-12 sm:py-16 md:py-20 lg:py-24 relative overflow-hidden" style="background: linear-gradient(180deg, #ffffff 0%, #f0fdf4 100%);">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div x-data="{ show: false }" x-init="setTimeout(() => show = true, 300)" 
                ...
                <div class="flex flex-col items-center text-center">
                    <div class="relative w-48 h-48 sm:w-56 sm:h-56 lg:w-64 lg:h-64 mb-6">
                        <picture>
                            @if($principalImageWebP)
                                <source srcset="{{ $principalImageWebP }}" type="image/webp">
                            @endif
                            <img 
                                src="{{ $principalImage }}" 
                                alt="{{ $principal->name ?? 'Principal' }}"
                                class="w-full h-full rounded-2xl object-cover shadow-lg hover:shadow-xl transition-shadow duration-500 ring-4 ring-[#008236]/20"
                                loading="lazy"
                            >
                        </picture>
                    </div>
                    ...
                    <div class="space-y-2">
                        <h3 class="text-2xl sm:text-3xl font-bold text-[#008236]">
            ...
            <div x-data="{ show: false }" x-init="setTimeout(() => show = true, 700)" 
                 class="mt-12 pt-8 border-t-2 border-gray-200 opacity-0 transform translate-y-8 transition-all duration-1000 delay-500 ease-out"
                 :class="{ 'opacity-100 translate-y-0': show }">
                <div class="flex items-center gap-4">
                    <div>
                        <p class="text-lg font-bold text-[#008236]">{{ $principal->name }}</p>
                        @if($principal->position)
                        <p class="text-sm text-gray-600">{{ $principal->position }} <br> 
                        Duha International School <br>
                        Chittagong
