...
>
    ...
    @php
        ...
        $phone = \App\Helpers\SiteSettingsHelper::primaryPhone() ?: '+880 1748806492';
        $email = \App\Helpers\SiteSettingsHelper::primaryEmail() ?: 'Duhaacademy.bd@gmail.com';
    ...
    <div class="text-white py-2 text-xs relative z-60" style="background-color: #008236;">
        ...
        <div class="lg:hidden">
            <div class="px-4 py-2 flex items-center justify-between">
                ...
                {{-- Toggle Button for Social/Contact --}}
                <button
                    @click="topBarExpanded = !topBarExpanded"
                    class="ml-2 p-1 text-white/90 hover:text-white transition-colors"
                    aria-label="Toggle contact information"
                >
                    <svg class="w-4 h-4 transition-transform duration-200" :class="{ 'rotate-180': topBarExpanded }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
            </div>

            {{-- Collapsible Social/Contact Section --}}
            <div
                x-show="topBarExpanded"
                ...
                <div class="hidden lg:flex items-center gap-6">
                    @foreach($navigationItems as $navItem)
                        @php
                            ...
                            $linkAttrs = '';
                            // For external links, don't open in new tab (same window per PRD)
                            if ($navItem->is_external) {
                                // External link - no target_blank, opens in same window
                                $linkAttrs = '';
                            } elseif ($navItem->target_blank) {
            ...
            @foreach($navigationItems as $navItem)
                ...
                @php
                    ...
                    if ($navItem->is_external) {
                        ...
                        $mobileLinkAttrs = 'target="_blank" rel="noopener noreferrer"';
                    }
                @endphp
                ...
                @if($hasChildren)
                    ...
                    <div x-data="{ open: false }" class="space-y-1">
                        <button
                            @click="open = !open"
                            class="flex items-center justify-between w-full px-4 py-3 text-base font-semibold rounded-lg transition-colors min-h-[44px]"
                        ...
                        <div 
                            ...
                            @foreach($children as $child)
                                @php
                                    $childMobileLinkAttrs = '';
                                    if ($child->is_external) {
                                        $childMobileLinkAttrs = '';
                                    } elseif ($child->target_blank) {
                                        $childMobileLinkAttrs = 'target="_blank" rel="noopener noreferrer"';
                                    }
                                @endphp
                                <a
                                    href="{{ $child->url ?? '#' }}"
                                    @click="mobileMenuOpen = false"
                                    class="block px-4 py-2.5 text-sm text-gray-600 hover:bg-za-green-50 hover:text-za-green-primary rounded-lg transition-colors min-h-[44px]"
                                    {!! $childMobileLinkAttrs !!}
                                >
                                    {{ $child->title }}
                                </a>
                ...
                @else
                    ...
                    <a
                        href="{{ $navUrl }}"
                        @click="mobileMenuOpen = false"
                        class="block px-4 py-3 text-base font-semibold rounded-lg transition-colors min-h-[44px]"
                        :class="{{ $isActiveMobile ? 'true' : 'false' }} ? 'bg-za-green-light text-za-green-primary' : 'text-gray-700 hover:bg-gray-100'"
                        {!! $mobileLinkAttrs !!}
                    >
                        {{ $navItem->title }}
