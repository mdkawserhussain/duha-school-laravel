...
<div class="profile-card bg-white rounded-3xl shadow-md overflow-hidden border border-slate-200/80 hover:shadow-2xl hover:scale-105 transition-all duration-500">
    ...
    <div class="p-4 sm:p-6">
        ...
        <p class="text-blue-600 font-medium mb-2 sm:mb-3 text-sm sm:text-base">
            {{ $staff->position }}
        </p>
        ...
        <div class="mt-3 sm:mt-4">
            <a href="{{ route('staff.show', $staff->id) }}" class="text-blue-600 hover:text-blue-800 font-medium text-sm transition min-h-[44px] flex items-center">
                View Profile →
            </a>
        </div>

        <div class="flex flex-col space-y-2 mt-3 sm:mt-4">
            @if($staff->email)
            <div class="flex items-center text-sm text-gray-600 min-h-[44px]">
                <svg class="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                </svg>
                <a href="mailto:{{ $staff->email }}" class="hover:text-blue-600 transition duration-300 break-all">
                    {{ $staff->email }}
                </a>
            </div>
            @endif
        ...
        <div class="flex flex-wrap gap-3 mt-3 sm:mt-4">
            ...
            <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer"
               class="text-gray-400 hover:text-blue-600 transition duration-300 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-gray-100">
